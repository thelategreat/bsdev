...
function reload()
{
	...
	$.post('/admin/media/browser', 
		{ path: '/event/<?= $event->id ?>', 'slot': $('#slot_select').val() },
		function(data) {
...
<?php foreach( explode(",",$event->slots) as $slot ) { 
	if( !empty($slot) && strlen(trim($slot)) ) { ?>
			<option><?=$slot?></option>
<?php } 
} ?>
</select><button onclick="reload()"><img width="16" src="/img/reload.png" /></button>
<button onclick="MediaBrowser.init({path: '/event/<?=$event->id?>'});"><img src="/img/add.png" /></button>
	...
	<tr>
		<td>
			<form method="post" action="/admin/media" enctype="multipart/form-data" >
				...
				<input type="hidden" name="next" value="/admin/event/edit/<?=$event->id?>/media" />
				<input type="hidden" name="path" value="/event/<?=$event->id?>" />
