...
function reload()
{
	...
	$.post('/admin/media/browser', 
		{ path: '/films/<?= $film->id ?>', 'slot': $('#slot_select').val() },
		function(data) {
	...
	<tr>
		<td>
			<form method="post" action="/admin/media" enctype="multipart/form-data" >
				...
				<input type="hidden" name="next" value="/admin/films/edit/<?=$film->id?>/media" />
				<input type="hidden" name="path" value="/films/<?=$film->id?>" />
