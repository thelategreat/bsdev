...
function reload()
{
	...
	$.post('/admin/media/browser', 
		{ path: '/articles/<?= $article->id ?>', 'slot': $('#slot_select').val() },
		function(data) {
...
<button onclick="MediaBrowser.init({path: '/articles/<?=$article->id?>'});"><img src="/img/add.png" /></button>
...
<div id="upload_div" >
	...
	<form method="post" action="/admin/media" enctype="multipart/form-data" >
		...
		<input type="hidden" name="next" value="/admin/articles/edit/<?=$article->id?>/media" />
		<input type="hidden" name="path" value="/articles/<?=$article->id?>" />
