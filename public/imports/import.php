...
foreach ($dirArray as $file) {
	...
	if ($ext != '.tab') continue;
	if (preg_match('/product_2.*tab/', $file))  {
		$products_files[] = $file;
	}
}
...
foreach ($products_files as $file) {
	...
	while ($row = fgetcsv($handle, 0, "\t")) {
		...
		$record['record_types_id'] = getLinkedTableId('record_types', $row[$j++], 'name', false);
		$record['record_source_id'] = getLinkedTableId('record_sources', $row[$j++], 'name', true);
		$record['title'] = $row[$j++];
		$record['contributors'] = explodeOn('|', $row[$j++]);
		if (count($record['contributors']) > 1) {
			//var_dump($record['contributors']);
		} else {
			//continue;
		}
		...
		$record['publish_date'] = date('Y-m-d', strtotime(trim($row[$j++])));
		...
		$record['size'] = $row[$j++];
		$record['awards'] = $row[$j++];
		$record['audiences_id'] = getLinkedTableId('audiences', $row[$j++], 'name', false);
...
/*
echo '---';
//var_dump($record['contributor_ids']);
echo '<hr>';
var_dump($r['contributor_ids']);
die;
*/
	...
	for ($i=0; $i<count($r['othertext_ids']); $i++) {
		...
		$sql = "INSERT INTO product_othertext (products_id, othertext_codes_id, othertext_id)
				VALUES ('{$id}', '{$code}', '{$text}')";
		$query = $db->query($sql);
